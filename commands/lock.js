...
module.exports = {
	...
	data: new SlashCommandBuilder()
		.setName('lock')
		.setDescription('Lock the account of a player with driver name or email.')
			...
			con.query("SELECT ID, email, isLocked FROM USER WHERE email = ?", target, (err, result) => {
				if (result.length > 0) {
					con.query("SELECT ID, IconIndex FROM PERSONA WHERE USERID = ? ORDER BY ID LIMIT 1", result[0].ID, (err2, result2) => {
						con.query("UPDATE USER SET isLocked = 1 WHERE ID = ?", result[0].ID)
						var icon = result2[0].iconIndex + config.url.avatarFormat
						const embed = new EmbedBuilder()
						if (result2[0].ID === 0) {
							embed.setAuthor({
								name: "The account with email " + result[0].email + " has been locked.",
